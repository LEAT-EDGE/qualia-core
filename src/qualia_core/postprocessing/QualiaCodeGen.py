        ...
        for node in modelgraph.nodes:
            if node.layer.name in activations_range:
                node.q = Quantization(
                        ...
                        weights_width=activations_range[node.layer.name].weights_bits,
                        bias_width=activations_range[node.layer.name].bias_bits,
                        output_width=activations_range[node.layer.name].activation_bits,
        ...
        if self._number_type is int: # Activation range only when using fixed-point quantization
            ...
            for node in modelgraph.nodes:
                ...
                node.q = Quantization(
                        ...
                        width=self.__width,
                        long_width=self.__long_width,
                        weights_width=self.__width,
                        bias_width=self.__width,
